...
from customer_service.app.core.client import client
from customer_service.app.mcp.generate_agent_tool import MCPTool
from customer_service.app.mcp.mcp_operator import MCPOperator
...
class AfterSalesAgent:
    ...
    async def generate_after_sales_agent(self, query):
        await self.generate_tools()
        print(self.order_tools)
        order_agent = create_react_agent(model=client.async_openai_chat, tools=self.order_tools, prompt=self.prompt)
        result = await order_agent.ainvoke({"messages": [{"role": "user", "content": query}]})
