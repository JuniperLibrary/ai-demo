...
"""
这是一个 基于 FastAPI 的股票分析 API，主要功能是：

1. 接收股票代码和市场类型作为输入。
2. 从 akshare 拉取股票或基金历史数据。
3. 计算一系列 技术指标（均线、RSI、MACD、布林带、成交量、ATR 等）。
4. 根据这些指标对股票进行 评分 并生成投资建议。
5. 提供一个 /analyze-stock/ 接口，返回技术指标概要、近期数据和分析报告。
6. 内置了简单的 Token 鉴权。
"""
...
# 参数配置：各类技术指标的默认参数
params = {
    'ma_periods': {'short': 5, 'medium': 20, 'long': 60},  # 均线周期
    'rsi_period': 14,  # RSI周期
...
def verify_auth_token(authorization: str = Header(None)):
    """
    验证 HTTP Header 中的 Authorization 是否为合法 Bearer Token
    """
    print(authorization)
    ...
    # 这里是模拟验证逻辑：可替换为数据库或Redis校验
    valid_tokens = ["xue123", "xue1234"]  # 有效 Token 列表
...
class StockAnalysisRequest(BaseModel):
    ...
    """
    stock_code: str  # 股票代码
    market_type: str = 'A'  # 市场类型（A股/HK/US/ETF/LOF）
    start_date: str = None  # 起始日期（默认过去一年）
...
def calculate_score(df):
    """
    根据技术指标计算评分
    总分 100分，分项包括趋势、RSI、MACD、成交量
    """
    try:
        score = 0
        latest = df.iloc[-1]  # 最新一条数据
